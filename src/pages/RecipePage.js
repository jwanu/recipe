...
const RecipePage = ({ id }) => {
  const [detail, setDetail] = useState(fakeDetail);
  const [equip, setEquip] = useState(fakeEquip.equipment);
  const [priceBreak, setPrice] = useState(fakePB);
  const [instructions, setInstructions] = useState(fakeInst);
